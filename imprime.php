...
?>
 ...
 <?php
	...
	$consul = "SELECT ci,nro_credito FROM formulario where id_form='$id'";
	$re = $mysqli->query($consul);

	$ro = $re->fetch_assoc();
	$cise=$ro['ci'];
	$nrose=$ro['nro_credito'];


	$query = "SELECT concepto, monto,respaldo FROM detalle where id_form='$id'";
	...
	$query3 = "SELECT actividad, id_form, fecha, total FROM formulario where id_form='$id'";
	...
	$pdf->Cell(70,6,' '.utf8_decode($row1['nombres']).' '.utf8_decode($row1['apellidos']),0,0,'D',1);
	...
	$pdf->Cell(20,6,$row3['total'],1,1,'C',1);
	$pdf->Output();
?>